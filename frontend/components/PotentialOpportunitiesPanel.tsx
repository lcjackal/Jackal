    ...
    <div className="opportunity-panel">
      ...
      <ul>
        {opportunities.map((o, i) => {
          const shortSummary = o.sources.map(s => s.type).join(", ");
          const fullDetailText = o.sources
            .map(s => `${s.type}${s.detail ? `: ${s.detail}` : ""}`)
            .join(" | ");
          return (
            <li key={i} style={{ marginBottom: 10 }}>
              <b>{o.coin}</b>: 
              <span title={fullDetailText}>
                {" "}{shortSummary}
              </span>
              kaynaklarına göre <b>{o.expected_time}</b> içinde <b>{o.price_target} USDT</b> potansiyeli.
